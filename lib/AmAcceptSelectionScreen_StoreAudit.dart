...
class AmAcceptSelectionScreen_StoreAudit extends StatefulWidget {
  ...
  AmAcceptSelectionScreen_StoreAudit(this.activeCheckList, this.mLpdChecklist);

  @override
  State<AmAcceptSelectionScreen_StoreAudit> createState() =>
      _AmAcceptSelectionScreen_StoreAuditState(
          this.activeCheckList, this.mLpdChecklist);
}

class _AmAcceptSelectionScreen_StoreAuditState
    extends State<AmAcceptSelectionScreen_StoreAudit> {
...
*/
                                                  ...
                                                  Row(
                                                    children: [
                                                      ...
                                                      Text(
                                                          ...
                                                          '${mAmHeaderQuestion[pos].updatedBy} - ${mAmHeaderQuestion[pos].updatedBy}- $time_',
                                                          style: const TextStyle(
                                                              fontSize: 14,
                                                              color: Colors.black,
                                                              fontWeight:
                                  ...
                                  children: [
                                    ...
                                    Column(
                                      crossAxisAlignment:
                                          CrossAxisAlignment.start,
                                      children: [
                                        const Text('Department rating'),
                                        RatingBar.builder(
                                          // tapOnlyMode: false,
                                          initialRating: ratingValue_2,
                                          direction: Axis.horizontal,
                                          allowHalfRating: true,
                                          itemCount: 5,
                                          itemPadding: const EdgeInsets.symmetric(
  ...
  List<AmHeaderQuestionStoreAudit> mAmHeaderQuestion = [];
  List mAmHeaderQuestion_notSelected = [];
  List<HeaderQuesStoreAudit> headerQuestionSelected = [];
...
//${widget.mLpdChecklist.sectionId}
      String url =
...
/*    HeaderQuestion headerQuestionFromJson(String str) => HeaderQuestion.fromJson(json.decode(str));
    ...
    if (mAmHeaderQuestion.length != 0) {
      ...
      for (int j = 0; j < mAmHeaderQuestion[i].checkListDetails.length; j++) {
        ...
        if (mAmHeaderQuestion[i].checkListDetails[j].imageName != "") {
          ...
          print(
              'imageUrl $locationCode->${mAmHeaderQuestion[i].checkListDetails[j].imageName}');
  ...
  Future<void> sendData__() async {
    ...
    try {
      ...
      '/RWA_GROOMING_API/api/StoreAudit/QuestionUpdate',
      );
        ...
        if (headerQuestionSelected_.isNotEmpty ||
          ...
          for (int i = 0; i < headerQuestionSelected_.length; i++) {
            ...
            params.add({
              "store_checklist_assign_id": mAmHeaderQuestion.isEmpty
                  ? 0
                  : mAmHeaderQuestion[pos].storeChecklistAssignId,
              "checklist_id": widget.activeCheckList.checklisTId,
              "checklist_item_mst_id": mAmHeaderQuestion.isEmpty
                  ? 0
  ...
  Future<void> sendData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (respo['statusCode'] == "200") {
        ...
        // if()
        _showSuccessAlert('Workflow status updated');
        // Navigator.pop(context);
      } else {
        setState(() {
          loading = false;
