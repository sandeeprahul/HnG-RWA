...
class checkInOutScreenDilo extends StatefulWidget {
  // int chekassignId_;
  ...
  @override
  State<checkInOutScreenDilo> createState() => _checkInOutScreenDiloState(
      this.activeCheckList, this.mGetActvityTypes, this.locationsList);
}

class _checkInOutScreenDiloState extends State<checkInOutScreenDilo> {
  ActiveCheckList activeCheckList;
  final GetActvityTypes mGetActvityTypes;
  final String locationsList;

  _checkInOutScreenDiloState(
      this.activeCheckList, this.mGetActvityTypes, this.locationsList);
  ...
  Future<void> getlocation(var v) async {
    try {
      setState(() {
        loading = true;
      });
      ...
      setState(() {
        lat = double.parse(widget.activeCheckList.latitude);
        lng = double.parse(widget.activeCheckList.longitude);
      ...
      // var latt = prefs.getString('latitude');
      // var lngg = prefs.getString('longitude');
      var latt = position.latitude; //user lat

      var lngg = position.longitude; //user lng
      setState(() {
        loading = false;
      });
//16.4704458,80.6064535
      checkDistance_(latt, lngg, lat_, lng_, v);
    } catch (e) {
  ...
  checkDistance_(var lat, var lng, var lat_, var lng_, var firstime) async {
    try {
      ...
      double distanceInMeters = Geolocator.distanceBetween(
          lat_, lng_, currentLatitude, currentLongitude);
      ...
      if (double.parse(meters) <= 150) {
        ...
        alertReCheck('You are outside of store\n$meters meters far', -1);
      }
    } catch (e) {
      Navigator.pop(context);
    }
  ...
  Future<void> cloudstorageRef(var img, var empCCode_) async {
    ...
    final imagesRef =
        storageRef.child("$locationCode/DiloCheckIn/$empCode.jpg");
...
// Create a reference to 'images/mountains.jpg'
  ...
  checkInUser(var base64img) async {
    try {
      ...
      var params = {
        "checklist_assign_id": activeCheckList.checklistAssignId,
        "userId": userId,
        "check_in_lat": '$lat',
        "check_in_long": '$lng',
        "created_by": 0,
